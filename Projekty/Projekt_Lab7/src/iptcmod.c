...
struct rule* parse(int argc, char ** argv) {
    ...
    while (*argv) {
		...
		else if (!strcmp(*argv, "-P"))
		{
			...
			/*
             * Przejscie do argumentu po '-P <chain>'
             * (typ polityki do ustawienia - ACCEPT lub ):
             */
            argv++;
            if ((*argv == NULL) || (**argv == '-')) {
                parse_error("Expecting <policy type - ACCEPT or DROP> after "
                            "-P <chain>\n", &error);
                break;
            }

			if(!strcmp(*argv, "ACCEPT"))
			{
				r->operation = POLICY_ACCEPT;
			}
			else if(!strcmp(*argv, "DROP"))
			{
				r->operation = POLICY_DROP;
			}
			else
			{
				parse_error("Expecting ACCEPT or DROP\n", &error);
				break;
			}
        }
...
void delete_rule(struct xtc_handle *h, struct rule* r) {
    ...
    retval = iptc_delete_num_entry(r->chain, r->rule_num, h);
    if (!retval) {
...
void policy_accept(struct xtc_handle *h, struct rule* r)
{
	/* Sprawdzenie czy podany lancuch wystepuje w tablicy: */
	if(!iptc_is_chain(r->chain,h))
	{
...
void policy_drop(struct xtc_handle *h, struct rule* r)
{
	/* Sprawdzenie czy podany lancuch wystepuje w tablicy: */
	if(!iptc_is_chain(r->chain,h))
	{
