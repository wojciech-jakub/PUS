...
int main(int argc, char **argv) {
    ...
    char* sciezka = (char*)malloc(2048);
    memset(sciezka,0,sizeof(sciezka));

    while(!cHandle)
    {
        fprintf(stdout,"Podaj nazwe pliku do pobrania: ");
        scanf("%s",filename);

        //uzyskiwanie sciezki
        strncpy(sciezka,dir, sizeof(dir));
        strncat(sciezka,"/", 1);
        strncat(sciezka,filename, sizeof(filename));

        cHandle = libssh2_sftp_open(sftp, sciezka, LIBSSH2_FXF_READ, 0);

    }
    fprintf(stderr, "\nPlik zostal pobrany prawidlowo, sciezka: %s\n", sciezka);
    ...
    while(1)
    {
        reader = libssh2_sftp_read(cHandle, buff, sizeof(buff));
        if (reader > 0){
            write(file, buff, reader);
        }
        else
            break;
    }
