...
int authenticate_user(LIBSSH2_SESSION *session, struct connection_data *cd) {
    ...
    /* Sprawdzenie czy serwer obsluguje metode klucza publicznego. */
    ...
    {
        /*
        Wprowadzić hasło uzyte do zabezpieczenia klucza prywatnego (jeseli zostało
        podane podczas generowania kluczy).
        */
        printf("Czy klucz RSA posiada haslo? (y/n): ");
        char odpowiedz;
        ...
        while(sprawdz)
        {
            ...
            else // reszte traktuje jako odp "no"
            {
                *password = NULL;
                sprawdz=0;
            }
        ...
        if (libssh2_userauth_publickey_fromfile(session, cd->username, "./public.key", "./private.key", password) == 0) {
